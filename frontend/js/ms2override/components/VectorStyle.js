...
const Icons = require('../../../MapStore2/web/client/utils/openlayers/Icons');

const defaultStyles = {
  'Point': () => [new ol.style.Style({
      image: image
  })],
  'LineString': (options) => [new ol.style.Style({
  ...
  'MultiLineString': (options) => [new ol.style.Style({
    stroke: new ol.style.Stroke(options.style.stroke ? options.style.stroke : {
        color: options.style.color || 'blue',
        width: options.style.weight || 1,
        opacity: options.style.opacity || 1
  ...
  'MultiPolygon': (options) => [new ol.style.Style({
    stroke: new ol.style.Stroke(options.style.stroke ? options.style.stroke : {
        color: options.style.color || 'blue',
        width: options.style.weight || 1,
        opacity: options.style.opacity || 1
    ...
    fill: new ol.style.Fill(options.style.fill ? options.style.fill : {
        color: options.style.fillColor || 'rgba(0, 0, 255, 0.1)',
        opacity: options.style.fillOpacity || 1
    })
  })],
  'Polygon': (options) => [new ol.style.Style({
    stroke: new ol.style.Stroke(options.style.stroke ? options.style.stroke : {
        color: options.style.color || 'blue',
        width: options.style.weight || 1,
        opacity: options.style.opacity || 1
